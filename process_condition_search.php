            ...
            <table cellspacing="25" style='margin-left: 34.5%; margin-top: 0px; margin-bottom: 10px;'>
                ...
                <?php
                    $conn = mysqli_connect(
                        ...
                        'carnumbers');
                    // echo $_POST['cond1'];
                    if($_POST['cond1'] == 'C_num'){
                        $sql = "SELECT * FROM topic WHERE car_number LIKE '%{$_POST['input']}%'"; 
                    ...
                    }else if($_POST['cond1'] == 'C_name'){
                        $sql = "SELECT * FROM topic WHERE name LIKE '{$_POST['input']}'"; 
                        $result = mysqli_query($conn, $sql);
                        if (mysqli_num_rows($result) > 0) {
                            while($row = mysqli_fetch_assoc($result)) {
                                echo "<tr><td align = 'center'>" . $row['name']."</td>";
                    ...
                    }else if($_POST['cond1'] == 'C_phone'){
                        $sql = "SELECT * FROM topic WHERE phone_number LIKE '{$_POST['input']}'"; 
                        $result = mysqli_query($conn, $sql);
                        if (mysqli_num_rows($result) > 0) {
                            while($row = mysqli_fetch_assoc($result)) {
                                echo "<tr><td align = 'center'>" . $row['name']."</td>";
                    ...
                    }else if($_POST['cond1'] == 'C_date'){
                        $sql = "SELECT * FROM topic WHERE date LIKE '%{$_POST['input']}%'"; 
                        $result = mysqli_query($conn, $sql);
                        if (mysqli_num_rows($result) > 0) {
                            while($row = mysqli_fetch_assoc($result)) {
                                echo "<tr><td align = 'center'>" . $row['name']."</td>";
